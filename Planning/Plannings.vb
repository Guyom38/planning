...
Public Class CL_Plannings
    Class Key_Reservation
        ...
        Public Groupe As ENUM_Mode_Groupe = ENUM_Mode_Groupe.Tout
        Public Semaine As ENUM_Mode_Groupe = ENUM_Mode_Groupe.Tout
    End Class
    ...
    Class CL_DONNEES
        ...
        Public Liste_Creneaux As Dictionary(Of Key_Reservation, Objet_Creneau)
    ...
    Public Function Recuperer_Liste_Creneaux(_key As Key_Reservation) As Dictionary(Of Key_Reservation, Objet_Creneau)
        Dim tmp_liste As New Dictionary(Of Key_Reservation, Objet_Creneau)

        For Each creneau In DONNEES.Liste_Creneaux
            Dim cle As Key_Reservation = creneau.Key

            If cle.Enseignant = _key.Enseignant _
                OrElse cle.Classe = _key.Classe _
                OrElse cle.Salle = _key.Salle Then

                tmp_liste.Add(cle, creneau.Value)
            End If
        Next
    ...
    Public Sub Afficher_Verrou()
        Dim ligne As String = ""
        For y = 1 To GRILLE.CELLULES.GetLength(1) - 1
            ligne = ""
            For x = 1 To GRILLE.CELLULES.GetLength(0) - 1
                Dim m As String = CStr(IIf(GRILLE.CELLULES(x, y).Init, "O", "X"))


                ligne &= CStr(IIf(m > "", "[" + Space(3 - Len(m)) & m & "] ", "[" & Space(3) & "] "))
            Next
    ...
    Public Sub Preparer_Grille(Optional nom_planning As String = "", Optional _export As Boolean = False)
        ...
        Dim cle As New Key_Reservation() With {.Classe = }

        Dim liste_des_crenaux As Dictionary(Of Key_Reservation, Objet_Creneau) = Recuperer_Liste_Creneaux()
        ...
        For Each ps In liste_construction
            ...
            For y = 0 To GRILLE.CELLULES.GetLength(1) - 1
                Dim heure As String = ""
                If (y > 0) Then heure = LISTE_HEURES(y - 1)

                For x = 0 To GRILLE.CELLULES.GetLength(0) - 1
                    Dim jour As String = ""
                    ...
                    If x > 0 And y > 0 Then
                        ...
                        If (GRILLE.CELLULES(x, y).Init And creneau.Active(i)) Or ps Is planning_select Then
    ...
    Public Sub Prepare_Fusion_Cellules(ByRef _psSelect As Objet_Planning, ByRef _ps As Objet_Planning, ByRef _creneau As Objet_Creneau, x As Integer, y As Integer)
        With GRILLE.CELLULES(x, y)
            ...
            If Not _ps Is _psSelect Then
                .FusionX = 2
                If QuelGroupe(x) = ENUM_Mode_Groupe.Groupe2 Then x = x - 1
            End If
            ...
            For ny = 0 To .FusionY - 1
                For nx = 0 To .FusionX - 1
                    ...
                    If x + nx < GRILLE.CELLULES.GetLength(0) And y + ny < GRILLE.CELLULES.GetLength(1) Then
                        GRILLE.VERROUS(x + nx, y + ny) = x & "," & y
    ...
    Public Function Verification_Creneaux_Disponibles(_x As Integer, _y As Integer, _deuxGroupes As Boolean, _duree As Integer, _creneau As Objet_Creneau) As Boolean
        For yy = _y To _y + _duree - 1
            For xx = _x To _x + CInt(IIf(_deuxGroupes, 1, 0))
                If xx > GRILLE.CELLULES.GetLength(0) - 1 Or yy > GRILLE.CELLULES.GetLength(1) - 1 Then
                    Debug.Print("En dehors")
                    Return False
                Else
                    If Not GRILLE.VERROUS(xx, yy) = "" Then
                        Debug.Print("Occupé")
                        Return False
                    End If
                End If
